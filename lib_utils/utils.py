...
def find_best_threshold(xs, ys):
    # xs, ys are 1-d numpy arrays, ys∈{0,1}
    order = np.argsort(xs)
    xs, ys = xs[order], ys[order]
    # initial errors if threshold < min(xs): all ones on right mispredicted
    FN = np.sum(ys==0)   # false negatives on right
    FP = 0               # false positives on left
    best_err = FN + FP
    best_t   = xs[0] - 1e-6
    # sweep threshold past each point
    for i in range(len(xs)):
        # move x_i from right to left of threshold
        if ys[i]==0:
            FN -= 1
            FP += 1
        else:  # ys[i]==1
            ...
            FP -= 0
        err = FP + FN
        if err < best_err and i+1 < len(xs):
            best_err, best_t = err, (xs[i]+xs[i+1])/2

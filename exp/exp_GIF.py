...
class ExpGraphInfluenceFunction(Exp):
    def __init__(self, args):
        ...
        for run in range(self.args['num_runs']):
            ...
            old_params  = [p.detach().clone() for p in self.target_model.model.parameters() if p.requires_grad]
            
            all_old_params = torch.cat([p.flatten() for p in old_params])
            mn, mx = torch.min(all_old_params), torch.max(all_old_params)
            ...
            new_params  = [p.detach().clone() for p in self.target_model.model.parameters() if p.requires_grad]

            #inf_norm = lambda a, b: torch.max(torch.abs(a - b))

            #mx_change = -1
            #for p1, p2 in zip(old_params, new_params):
            #    mx_change = max(mx_change, inf_norm(p1.flatten(), p2.flatten()))
            #    print(inf_norm(p1.flatten(), p2.flatten()))
            #print("range=[{:.4f}, {:.4f}], change={:.4f}, bits={}".format(mn, mx, mx_change, int((mx - mn) / mx_change)))
            #input()
    ...
    def unlearning_request(self):
        ...
        if self.args["unlearn_task"] == 'edge':
            ...
            self.data.removed_edges = remove_edges
            
            torch.save(self.data, f'data_{self.args['unlearn_ratio']}_{self.args['dataset_name']}.pth')
